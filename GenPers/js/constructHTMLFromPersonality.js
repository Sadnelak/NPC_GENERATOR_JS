var constructHTML=function(){
	...
	for(var i=0;i<tabDom.length;i++){
		inner_html+="<tr><td class='domaine' title='"+tabDom[i].description+"' alt='"+tabDom[i].description+"'>"+tabDom[i].nom+"</td>";
		var tabCarac=tabDom[i].caracteres;
		inner_html+="<td><table>";
		for(var j=0;j<tabCarac.length;j++){
			inner_html+="<tr><td class='carac' title='"+tabCarac[j].description+"' alt='"+tabCarac[j].description+"'>"+tabCarac[j].nom+"<input type='number' style='display:none;' value='"+tabCarac[j].probaTotal+"' />";
